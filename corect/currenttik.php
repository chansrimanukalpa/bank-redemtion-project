...
if($row['status']==0){
	...
	echo '<p><a href="status.php?Cnic='.$row['Cnic'].'&status=1">Pending</a></p>';
}else{
	echo '<p><a href="status.php?Cnic='.$row['Cnic'].'&status=0">Send to Manager</a></p>';
...
<td><select class="btn-container" type="text" name="dropdown_menu" id="cselec">
   <?php
  ...
  ?>
   <option class="btn-container" value="<?php echo $row['name'];?>"><?php echo $row['name']; ?></option>
   <?php echo $row['assg_driver']?>
<?php }?>
</select></td>
<td><input type="submit" class="btn-container button background"value = assign name="assign"></td>
</form>
<?php
  $sql = "SELECT * FROM ticket where cdate= CURDATE()";
$result = mysqli_query($conn, $sql);
...
if(isset($_POST['assign'])){
  $driver=$_POST['dropdown_menu'];
  $sql2 ="UPDATE ticket SET assg_driver ='$driver' WHERE Cnic = '$Cin' ";
